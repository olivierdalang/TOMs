...
UPDATE public."Lines" SET
    ...
    "label_leader" = ST_Multi(ST_MakeLine(ST_Centroid("geom"), ST_SetSRID(ST_MakePoint("labelX", "labelY"), 27700))),
    "label_loading_leader" = ST_Multi(ST_MakeLine(ST_Centroid("geom"), ST_SetSRID(ST_MakePoint("labelLoadingX", "labelLoadingY"), 27700)));
...
UPDATE public."Bays" SET
    ...
    "label_leader" = ST_Multi(ST_MakeLine(ST_Centroid("geom"), ST_SetSRID(ST_MakePoint("label_X", "label_Y"), 27700)));
...
CREATE OR REPLACE FUNCTION update_leader_fct() RETURNS trigger SECURITY DEFINER AS $emp_stamp$
    declare
        i integer := 0;
        pnt public.geometry;
    BEGIN
